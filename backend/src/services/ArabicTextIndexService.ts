...
export class ArabicTextIndexService
{
    ...
    private AddNounToIndex(word: OpenArabDictLexeme, unit: OpenArabDictGendered, trie: PrefixTree<IndexEntry>)
    {
        ...
        if(isSingular && this.DoesChildExist(word.id, OpenArabDictParentType.Plural))
        {
            const dual = conjugator.DeriveSoundAdjectiveOrNoun(reconstructed, gender, TargetAdjectiveNounDerivation.DeriveDualSameGender, DialectType.ModernStandardArabic);
    ...
    private AddToIndex(indexEntry: IndexEntry, trie: PrefixTree<IndexEntry>)
    {
        ...
        const translations = this.translationIndexService.GetTranslationsOf(indexEntry.word.id, "en");
        for (const entry of translations)
        {
            ...
            {
                const vocalized = ArabicText.ParseVocalizedPhrase(usage.text);
                const key = this.MapPhraseToKey(vocalized);

                trie.Add(key, {
                    vocalized,
                    word: indexEntry.word,
                    derived: {
                        parent: { type: "i", kind: ImplicitWordDerivation.ExampleOrMeaningInContext },
                        text: usage.text
                    }
                });
            }
